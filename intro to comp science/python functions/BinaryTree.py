...
class BinaryTree:
    ...
    def sum_leaves(self):
        return self.sum_leaves_aux(self.root, 0)

    def sum_leaves_aux(self, current, sum):
        if current.left is not None:
            sum = self.sum_leaves_aux(current.left, sum)
        if current.right is not None:
            sum = self.sum_leaves_aux(current.right, sum)
